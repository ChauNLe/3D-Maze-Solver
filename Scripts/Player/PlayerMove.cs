...
void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.name == "rocky")
        {
            Debug.Log("HE");
    ...
    private void Update() // Called once per frame
    {
        ...
        if (FindObjectOfType<Timer>().timerIsRunning && !isGameOver && Input.GetKey(KeyCode.LeftShift)) // If the left shift button is being pressed
        {
            speed = runSpeed; // Set our speed to runSpeed
        }
        else if (FindObjectOfType<Timer>().timerIsRunning && !isGameOver && (Input.GetKey("up") || Input.GetKey("w")))
        {
